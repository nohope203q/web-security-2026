...
<html lang="vi">
    ...
    <body>
        ...
        <div class="main">
            <h1>
                <c:choose>
                    <c:when test="${viewMode == 'profane'}">Bình luận Chứa Từ ngữ Khiếm nhã</c:when>
                    <c:otherwise>Quản lý Bình luận</c:otherwise>
                </c:choose>
            </h1>
                        ...
                        <c:when test="${not empty reviews}">
                            <c:forEach var="r" items="${reviews}">
                                <tr>
                                    ...
                                    <td>
                                        <c:if test="${r.verified}"><span class="status-verified">Đã mua hàng</span></c:if>
                                        <c:if test="${not r.verified}"><span class="status-unverified">Chưa mua hàng</span></c:if>
                                        </td>
                                        <td>${r.comment}</td>
                                    <td><fmt:formatDate value="${r.createdAt}" pattern="HH:mm, dd/MM/yyyy" /></td>
                                    <td>
                                        <a href="${pageContext.request.contextPath}/admin/review?action=delete&id=${r.id}"
